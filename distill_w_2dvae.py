...
def main(args):
    ...
    project_name = "Latent_Video_w_2dvae"
    ...
    def get_images(c, n):  # get random n videos from class c
    ...
    # Save the syn_image into a pt file
    image_save = image_syn.detach()
    save_dir = os.path.join(args.save_path, project_name, wandb.run.name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    torch.save(image_save.cpu(), os.path.join(save_dir, "syn_videos_latent.pt"))
